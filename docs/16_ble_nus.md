...
### 광고가 안 보이는데 보드는 정상인 경우 (macOS)

`ble info` 가 `connected : True` 인데 스캔에 안 보이면 **아직 연결되어 있는 것**이다 (연결 중에는 광고하지 않는다).

macOS(CoreBluetooth)는 **보류 중인 연결 요청을 유지**한다. 호스트 프로그램이 `disconnect()` 를 기다리지 않고 끝나거나
연결 실패 경로에서 정리하지 않으면, 보드가 광고를 시작하는 순간 OS 가 자동으로 다시 붙는다.
실기에서 `ble disconnect` 로 끊으면 1초 뒤 광고가 살아났다가 2초 만에 다시 연결되는 것을 확인했다 (보드 리셋 직후에도 마찬가지).

→ 보드가 아니라 **호스트 쪽 프로세스를 정리**해야 한다. 보드에서 끊을 때는 `ble disconnect`.
