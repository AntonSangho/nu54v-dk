...
| 2026-09-25 | **baram-term 설치 + baram-ctl 로 uart 예제 시험** (Linux). `chcbaram/baram-term` 을 클론해 `uv venv --python 3.12` 로 설치(plain `pip` 는 `pyproject.toml` 의 `tool.uv.sources` 로컬 경로 재지정을 이해 못 해 `uv` 필요). Claude Code 플러그인(`claude plugin marketplace add chcbaram/baram-term` → `claude plugin install baram-term@baram`)도 설치, 새 세션부터 `baram-ctl` 사용 가능. VCOM1(cli) 은 USB 인터페이스 번호로 구분— `lsusb`/`udevadm info` 로 보면 `ID_USB_INTERFACE_NUM=03` 인 `/dev/ttyACM2` 가 cli(UART1), `01` 인 `/dev/ttyACM1` 이 VCOM0 이었다 (VCOM0 은 09_25 위 항목의 SWD 결함 대상이라 안 씀). `projects/uart` 를 빌드 후 `baram-ctl release` → `fw flash --probe <UID>` → `baram-ctl resume` 순서로 포트를 넘겨가며 플래시. `baram-ctl send "help"` 가 재연결 직후 첫 명령에서 응답 없이 씹힌 적이 있었는데 (재현: USB CDC 재열거 직후 첫 바이트 유실로 추정) 재시도하니 정상 — 펌웨어 버그 아님. `help`/`md`/`uart info` 로 VCOM1·VCOM0 둘 다 열려 있는 것까지 확인 |
